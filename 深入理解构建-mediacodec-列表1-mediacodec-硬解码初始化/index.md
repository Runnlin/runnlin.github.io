# 深入理解构建 MediaCodec 列表：1-MediaCodec 硬解码初始化
...
![](../assets/images/%E5%88%9D%E5%A7%8B%E5%8C%96%E8%BF%87%E7%A8%8B.jpg)
...
### 查找名称为 media.player 的服务
实际为 MediaPlayerService，然后通过 `MediaPlayerService getCodecList()` 获取到 `IMediaCodecList（BpMediaCodecList）`。这里跨进程通信实际是通过 `BpMediaPlayerService` 调用到远端的 `MediaPlayerService。`
...
```
### 插件提供可用的 OMX codec 列表
如果插件提供输入 surface，不能使用 OMX 视频编码器。
...
### MediaCodec cpp 构造函数以及其 init 流程
